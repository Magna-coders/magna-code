        ...
        {selectedConversation ? (
          <>
            ...
            <div className="flex items-center justify-between bg-[#111] border-b border-[#333] p-4">
              ...
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 rounded-full bg-[#FF9940]/20 flex items-center justify-center overflow-hidden">
                  {selectedConversation.participants.find(p => p.user_id !== currentUser?.id)?.users.avatar_url ? (
                    <img
                      src={selectedConversation.participants.find(p => p.user_id !== currentUser?.id)?.users.avatar_url}
                      alt={selectedConversation.participants.find(p => p.user_id !== currentUser?.id)?.users.username}
                      className="w-full h-full object-cover rounded-full"
                    />
                  ) : (
                    selectedConversation.participants.find(p => p.user_id !== currentUser?.id)?.users.username?.charAt(0).toUpperCase() || "?"
                  )}
                </div>
                <span className="font-medium text-[#F9E4AD]">
                  {selectedConversation.participants.find(p => p.user_id !== currentUser?.id)?.users.username || "Unknown"}
                </span>
              </div>
            </div>
